...
impl OIDCFilter {
    fn get_header(&self, name: &str) -> String {
        ...
        for (key, _value) in headers.iter() {
            if key.to_lowercase().trim() == name {
                debug!("get_headers:{}", _value.to_owned());
    ...
    fn get_code(&self) -> String {
        ...
        for (k, v) in encoded {
            if k == "code" {
                debug!("get_code method {}", v.to_owned().to_string() );
            ...
            if key.to_lowercase().trim() == "cookie" {
                ...
                for cookie_string in cookies {
                    ...
                    if cookie_name.trim() == name {
                        debug!("get_cookie {}", cookie_string[(cookie_name_end + 1)..cookie_string.len()]
                        .to_owned());
...
impl HttpContext for OIDCFilter {
    fn on_http_request_headers(&mut self, _: usize) -> Action {
        ...
        if code != "" {
            ...
            debug!("Code found. Dispatching HTTP call to token endpoint {} ", code.as_str());
            ...
            debug!("Token Request: {} ",self.config.auth_cluster.as_str());
            match token_request {
                Err(e) => {
                    self.send_error(
                        503,
