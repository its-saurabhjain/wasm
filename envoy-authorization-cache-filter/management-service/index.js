...
app.post("/cache", (req, res) => {
    ...
    if(cacheUpdate.isReset == false){
        ...
        console.log("cache reset update")
        cacheUpdate.foo_used = 0
        cacheableRules.foo_used = 0
        cacheUpdate.isReset = false
        cacheableRules.isReset = false
    }
    // If no change between local cache and global rules
...
app.get("/token", (req,res) => {
    ...
    cacheableRules.isReset = true
    console.log("Cache Reset")
    app.post("/cache", (req,res) => {
        req.body=cacheableRules
        res.status(200).send(cacheableRules)
      });
